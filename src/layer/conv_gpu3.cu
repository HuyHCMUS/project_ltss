...
void ConvGPU3::im2col(const Vector& image, Matrix& data_col) {
...
void ConvGPU3::forward(const Matrix& bottom) {
  ...
  for (int i = 0; i < n_sample; i ++) {
    ...
    // Tất cả đều giống với CONV_GPU2 nhưng đây sử dụng kerneltype = 2 (nhân với SMEM)
    matrix_multiplication(data_col_data, weight_data, result_data, m, n, k, streams[i], 2);
...
// data_col size: Matrix (hw_out, hw_kernel * channel_in)
// image size: Vector (height_in * width_in * channel_in)
void ConvGPU3::col2im(const Matrix& data_col, Vector& image) {
  int hw_in = height_in * width_in;
  int hw_kernel = height_kernel * width_kernel;
  int hw_out = height_out * width_out;
  // col2im
...
void ConvGPU3::backward(const Matrix& bottom, const Matrix& grad_top) {
  int n_sample = bottom.cols();
  grad_weight.setZero();
  grad_bias.setZero();
...
void ConvGPU3::update(Optimizer& opt) {
...
std::vector<float> ConvGPU3::get_parameters() const {
...
void ConvGPU3::set_parameters(const std::vector<float>& param) {
...
std::vector<float> ConvGPU3::get_derivatives() const {
